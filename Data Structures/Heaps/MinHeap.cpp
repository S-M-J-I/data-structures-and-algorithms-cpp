...
void Insert(int A[], int n)
{
    ...
    // while i>1 and while the temp variable is smaller than its parent
...
int Delete(int A[], int n)
{
    ...
    while(j < n-1){
        // check if right child is smaller than left child. If yes, then move j to the right child
        if(A[j+1] < A[j]) j++;
        // check if parent is greater than child, if yes
        if(A[i] > A[j]){
...
int main()
{
    ...
    int A[] = {0,10,8,20,3,6,15,2,25,18};
